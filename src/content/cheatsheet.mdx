...
<CommandTable commands={[
  ...
  { command: "FIRST_VALUE(col) OVER(...)", description: "Primer valor de la ventana" },
  { command: "LAST_VALUE(col) OVER(...)", description: "Ultimo valor (requiere ROWS UNBOUNDED FOLLOWING)" },
  { command: "SUM(col) OVER(ORDER BY ...)", description: "Suma acumulada (running total)" },
  { command: "PERCENT_RANK() OVER(...)", description: "Posicion relativa (0 a 1)" },
  { command: "CUME_DIST() OVER(...)", description: "Distribucion acumulada" },
]} />
